...
`consensus.app` + 手動PubMed + `NotebookLM` + `slide+Codia` の各工程を置き換え、既存の [ga-consensus-editor](../ga-consensus-editor)（Zotero連携）と `my_clone_for_textbook/style_engine`（教科書スタイル）を再利用する。
...
- `01_drafts/<date>_<slug>_textbook.md` … 本文ドラフト（`[!FIGURE]`付き、章末Vancouver参考文献）→ ga-consensus-editor で引用リンク確認
...
`assemble` 時に `03_assets/publication_checklist.md` の機械的サブセット（参考文献・出典番号・免責・COI・dry残留）を自動判定。医学的妥当性の最終判断は人/LLMレビューに委ねる設計。
